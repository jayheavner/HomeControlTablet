<template>
  <div>
    ...
    <div class="genres-view__inner">
      ...
      >
        <router-link :to="{ name: 'spotify-browse-genre', params: { id: item.id } }">
          <img
...
export default {
  ...
  methods: {
    ...
    async getCategoryPlaylists() {
      try {
        const response = await api.spotify.browse.getCategoryPlaylists(id);
        this.playlists = response.data.categories.items;
      } catch (e) {
        console.log(e);
      }
    }
